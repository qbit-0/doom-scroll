...
const CommentTree: React.FC<Props> = ({ comments, baseDepth, treeStartIndex, nlp }) => {
  ...
  const renderComment = (comment, commentStartIndex: number) => {
    switch (comment.kind) {
      case "more": {
  ...
  const renderBranch = (branch, key: number) => {
  ...
  const renderBranches = (branches) => {
    return branches.map((branch, index: number) => renderBranch(branch, index));
  };

  return <div>{renderBranches(branches)}</div>;
};
